...
void clan_changer()
{
	...
	if (Options::Menu.ColorsTab.ClanTag.GetState())
	{
		if (!m_local || !m_local->IsAlive() || !Interfaces::Engine->IsInGame() || !Interfaces::Engine->connected())
		{
			Tag2 += Tag.at(0);
			Tag2.erase(0, 1);
			set_clan_tag(Tag2.c_str(), " mirror gang ");
		}
		...
		if (GetTickCount() > lastTime)
		{
			Tag += Tag.at(0);
			Tag.erase(0, 1);
			set_clan_tag(Tag.c_str(), " mirror gang ");
...
bool __stdcall CreateMoveClient_Hooked(float frametime, CUserCmd* pCmd)
{
	...
	//	if (Options::Menu.MiscTab.AutoDefuse.GetState())
		...
		if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame() && pLocal)
		{
			...
			if (Options::Menu.MiscTab.slidewalk.GetState())
			{
		...
		MovementCorrection(pCmd, LocalPlayer); //slide fix
		...
		if (bSendPacket)
		{
			...
			current_desync = pCmd->viewangles.y;
		}
...
void hitsound()
{
	...
	case 6: PlaySoundA(hitler_wav, NULL, SND_ASYNC | SND_MEMORY);
		break;
	case 7: PlaySoundA(steam_sound1, NULL, SND_ASYNC | SND_MEMORY);
		break;
	}
}
...
bool __fastcall Hooked_FireEventClientSide(PVOID ECX, PVOID EDX, IGameEvent *Event)
{
	...
	if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame())
	{
		...
		if (event_name.find("round_prestart") != std::string::npos)
		{		
			warmup = true;			
		}
	...
	if (pLocalPlayer)
	{
		for (int i = 1; i <= Interfaces::Globals->maxClients; i++)
		{
			...
			if (ent && ent->GetClientClass()->m_ClassID == (int)CSGOClassID::CCSPlayer && ent->IsAlive() && !ent->IsDormant())
			{
				...
				if (ent->GetVelocity().Length2D() > 25 && pLocalPlayer->IsAlive() && pLocalPlayer->GetIndex() != ent->GetIndex() && ent->team() != pLocalPlayer->team() && Options::Menu.ColorsTab.BackTrackBones2.GetState())
				{
				...
				if (Options::Menu.ColorsTab.BackTrackBones.GetState() && teamnum != pLocalPlayer->team() && pLocalPlayer->IsAlive())
				{
					Color bt_color = Color(Options::Menu.ColorsTab.misc_backtrackchams.GetValue());
					...
					if (ent->GetVelocity().Length2D() > 1.f)
					{
						...
						static float kys[3] = { 0.f, 0.f, 0.f };
						kys[0] = bt_color.r() / 255.f;
						kys[1] = bt_color.g() / 255.f;
						kys[2] = bt_color.b() / 255.f;
		...
		if (Options::Menu.MiscTab.lby_helper.GetState())
		{
			static  IMaterial* CoveredLit = CreateMaterial(false, true, true);
			if (CoveredLit)
			{
				Vector abs = hackManager.pLocal()->GetAbsAngles();
				hackManager.pLocal()->SetAbsAngles(Vector(0, lineLBY, 0));

				float normal_color[4] = { 1, 0.1, 0.2, 1 };

				Interfaces::RenderView->SetColorModulation(normal_color);

				Interfaces::RenderView->SetBlend(0.3);

				Interfaces::ModelRender->ForcedMaterialOverride(CoveredLit);

				hackManager.pLocal()->draw_model(0x1, 255);

				Interfaces::ModelRender->ForcedMaterialOverride(nullptr);

				Interfaces::RenderView->SetBlend(1);

				hackManager.pLocal()->SetAbsAngles(Vector(0, abs.y, 0));
			}
		}
	}
}
...
void __fastcall Hooked_DrawModelExecute(void* thisptr, int edx, void* ctx, void* state, const ModelRenderInfo_t &pInfo, matrix3x4 *pCustomBoneToWorld)
{
	...
	if (pentity && pentity != pLocal && pentity->team() != pLocal->team() && strstr(ModelName, "models/player") )
	{
	...
	if (!pLocal->IsScoped() && strstr(ModelName, "models/player") && pentity != nullptr && pentity->team() == pLocal->team() && pentity == pLocal && Options::Menu.VisualsTab.ChamsLocal.GetState())
	{
	...
	if (pLocal->IsScoped() && strstr(ModelName, "models/player") && pentity != nullptr && pentity->team() == pLocal->team() && pentity == pLocal)
	{
	...
	if (HandsStyle != 0 && strstr(ModelName, XorStr("arms")) && pLocal )
	{
		switch (HandsStyle)
		{
		case 1:
...
void AnimFix(IClientEntity* entity)
{
	...
	bool is_teammate = local_player->team() == entity->team() && !is_local_player;
	if (is_local_player)
		return;
...
void AnimFix_ghetto()
{
	if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame())
	{
		...
		if (!local_player->IsAlive())
			return;
		//	if (local_player->GetFlags() & FL_ONGROUND && local_player->GetMoveType() & IN_WALK) 
		//		return;
		...
		local_player->SetAbsAngles(Vector(0.f, local_player->GetBasePlayerAnimState()->m_flGoalFeetYaw, 0.f));//if u not doin dis it f*cks up the model lol
	...
	if (Interfaces::Engine->IsConnected() && Interfaces::Engine->IsInGame())
	{
		...
		if (Options::Menu.MiscTab.OtherThirdperson.GetState() && pLocal)
		{
			...
			auto GetCorrectDistance = [&local_player](float ideal_distance) -> float
			{
				...
				Vector direction;
				//		VectorAngles( inverse_angles, direction );

				CTraceWorldOnly filter;
				trace_t trace;
				Ray_t ray;

				ray.Init(local_player->GetVecOrigin_2() + local_player->GetViewOffset(), (local_player->GetVecOrigin_2() + local_player->GetViewOffset()) + (direction * (ideal_distance + 5.f)));
