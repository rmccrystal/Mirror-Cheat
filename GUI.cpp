...
void CGUI::Update()
{
	...
	for (auto window : Windows)
	{
		...
		else
		{
			if (Options::Menu.VisualsTab.watermark.GetState())
			{
				Render::Text((centerW)-25, (centerh * 2) - 30, Color(250, 250, 250, (MenuAlpha - 1)), Render::Fonts::esp, "Mirror v5 Remastered");
				Render::Text((centerW)-1, (centerh * 2) - 17, Color(250, 250, 250, (MenuAlpha - 100)), Render::Fonts::esp, "by FreaK");
			}
		}
	}
}
...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	// ------ if you intend to use the old tabs, comment or delete everything from here until the next comment ------ //
	int iProportion = window->m_iWidth - 4 - 12 + 90 - 2;
	iProportion = iProportion / 3;

	Render::Clear(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2, window->m_iWidth - 4 - 12 + 90, window->m_iHeight - 2 - 8 - 26, Color(14, 14, 14, MenuAlpha));
	Render::Clear(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2 - 6, window->m_iWidth - 4 - 12 + 90, 6, Color(15, 15, 15, MenuAlpha));
	Render::Clear(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2 + window->m_iHeight - 2 - 8 - 26, window->m_iWidth - 4 - 12 + 90, 6, Color(17, 17, 17, MenuAlpha));
	Render::Clear(window->m_x + 8 - 90 - 6, window->m_y + 1 + 28 * 2 - 6, 6, window->m_iHeight - 2 - 8 - 26 + 12, Color(15, 15, 15, MenuAlpha));
	Render::Clear(window->m_x + 8 + window->m_iWidth - 4 - 12, window->m_y + 1 + 28 * 2 - 6, 6, window->m_iHeight - 2 - 8 - 26 + 12, Color(17, 17, 17, MenuAlpha));

	//Tab
	Render::Clear(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2, 90 - 16, window->m_iHeight - 2 - 8 - 26, Color(10, 10, 10, MenuAlpha));

	Render::Outline(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2, window->m_iWidth - 4 - 12 + 90, window->m_iHeight - 2 - 8 - 26, Color(23, 23, 23, MenuAlpha));
	//Render::Clear(window->m_x + 8, window->m_y + 1 + 28 * 2, 1, window->m_iHeight - 2 - 8 - 26 - 1, Color(129, 129, 129, 255));
	Render::Outline(window->m_x + 8 - 6 - 90, window->m_y + 1 + 28 * 2 - 6, window->m_iWidth - 4 - 12 + 90 + 12, window->m_iHeight - 2 - 8 - 26 + 12, Color(23, 23, 23, MenuAlpha));
	Render::Outline(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2, 90 - 16, window->m_iHeight - 2 - 8 - 26, Color(23, 23, 23, MenuAlpha));

	//Skeet Bar
	Render::GradientB(window->m_x - 81, window->m_y + 1 + 28 * 2 + 1, iProportion, 1, Color(outl_r, outl_g, outl_b, MenuAlpha), Color(inl_r, inl_g, inl_b, MenuAlpha), 1);
	Render::GradientB(window->m_x - 81 + iProportion, window->m_y + 1 + 28 * 2 + 1, iProportion, 1, Color(inl_r, inl_g, inl_b, MenuAlpha), Color(inr_r, inr_g, inr_b, MenuAlpha), 1);
	Render::GradientB(window->m_x - 81 + (iProportion * 2), window->m_y + 1 + 28 * 2 + 1, iProportion, 1, Color(inr_r, inr_g, inr_b, MenuAlpha), Color(outr_r, outr_g, outr_b, MenuAlpha), 1);
	...
	if (tabcount) // If there are some tabs
	{
		...
		for (int i = 0; i < tabcount; i++)
		{
			...
			if (window->SelectedTab == tab) {
				...
				Render::Clear(TabDrawArea.left, TabDrawArea.top, TabDrawArea.right, TabDrawArea.bottom, Color(14, 14, 14, MenuAlpha));
				Render::Line(TabDrawArea.left, TabDrawArea.top, TabDrawArea.left + TabDrawArea.right, TabDrawArea.top, Color(23, 23, 23, MenuAlpha));
				Render::Line(TabDrawArea.left, TabDrawArea.top + TabDrawArea.bottom, TabDrawArea.left + TabDrawArea.right, TabDrawArea.top + TabDrawArea.bottom, Color(23, 23, 23, MenuAlpha));
				Render::Text(TabDrawArea.left + (TabDrawArea.right / 2) - (TextSize.right / 2), TabDrawArea.top + (TabDrawArea.bottom / 2) - (TextSize.bottom / 2), Color(255, 255, 255, MenuAlpha), Render::Fonts::Tabs, tab->Title.c_str());
			}
			else {
				xAxis = window->m_x + 8 - (45 + TextSize.right / 2);
				Render::Text(TabDrawArea.left + (TabDrawArea.right / 2) - (TextSize.right / 2), TabDrawArea.top + (TabDrawArea.bottom / 2) - (TextSize.bottom / 2), Color(60, 60, 60, MenuAlpha), Render::Fonts::Tabs, tab->Title.c_str());
			}

			//Render::Clear(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, 2, Color(65, 55, 55, 255));
		}
	}
	// ------ if you intend to use the old tabs, comment or delete everything above me until the next comment ------ //
	...
	Render::Clear(window->m_x, window->m_y, window->m_iWidth, window->m_iHeight, Color(12, 12, 12, MenuAlpha));
	Render::Clear(window->m_x, window->m_y + 2 + 20, window->m_iWidth, 4, Color(18, 18, 18, MenuAlpha));
	//Tab
	int TabCount = window->Tabs.size();
	if (TabCount) // If there are some tabs
	{
	...
	Render::Outline(window->m_x - 2, window->m_y - 2, window->m_iWidth + 4, window->m_iHeight + 4, Color(21, 21, 21, MenuAlpha));
	Render::Outline(window->m_x - 3, window->m_y - 3, window->m_iWidth + 6, window->m_iHeight + 6, Color(21, 21, 21, MenuAlpha));
	Render::Outline(window->m_x - 4, window->m_y - 4, window->m_iWidth + 8, window->m_iHeight + 8, Color(21, 21, 21, MenuAlpha));
	Render::Outline(window->m_x - 5, window->m_y - 5, window->m_iWidth + 10, window->m_iHeight + 10, Color(21, 21, 21, MenuAlpha));
	Render::Outline(window->m_x - 6, window->m_y - 6, window->m_iWidth + 12, window->m_iHeight + 12, Color(30, 30, 30, MenuAlpha));
	Render::Outline(window->m_x - 7, window->m_y - 7, window->m_iWidth + 14, window->m_iHeight + 14, Color(12, 12, 12, MenuAlpha));
