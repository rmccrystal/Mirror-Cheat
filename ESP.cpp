...
void CEsp::Draw()
{
	...
	for (int i = 0; i < Interfaces::EntList->GetHighestEntityIndex(); i++)
	{
		...
		if (pEntity &&  pEntity != pLocal && !pEntity->IsDormant())
		{
			...
			if (Options::Menu.VisualsTab.FiltersPlayers.GetState() && Interfaces::Engine->GetPlayerInfo(i, &pinfo) && pEntity->IsAlive() && !pEntity->is_ghost())
			{
...
void CEsp::multipoint(IClientEntity* pEntity, Box size)
{
	if (Options::Menu.MiscTab.DebugHitbones.GetState()) {
		for (int i = 0; i < 19; i++) {
			...
			if (world_to_screenx(hitbone_pos, screen_position)) {
...
void CEsp::DrawWeapon2(IClientEntity* pEntity, Box size)
{
	IClientEntity* pWeapon = Interfaces::EntList->GetClientEntityFromHandle((HANDLE)pEntity->GetActiveWeaponHandle());
	if (pWeapon)
	{
		if (Options::Menu.VisualsTab.Weapons.GetIndex() == 1) {
			RECT nameSize = Render::GetTextSize(Render::Fonts::esp, pWeapon->GetWeaponName());
			Render::Text(size.x + (size.w / 2) - (nameSize.right / 2), size.y + size.h + 5,
				Color(Options::Menu.ColorsTab.Weaponsteam.GetValue()), Render::Fonts::esp, pWeapon->GetWeaponName());
		}
	}
}
void DrawSkeletonBECKTRECK(IClientEntity* entity, const matrix3x4_t* bone_matrix)
{
	...
	for (int i = 0; i < studio_model->numbones; i++)
	{
		...
		if (world_to_screenx(bone_position_1, screen1) && world_to_screenx(bone_position_2, screen2))
...
void CEsp::DrawAmmo(IClientEntity* pEntity, Box size)
{
	...
	if (!pEntity || pEntity == nullptr || !pEntity->IsAlive() || !pLocal || !pLocal->IsAlive())
		return;
...
void CEsp::DrawAmmo2(IClientEntity* pEntity, Box size)
{
	C_BaseCombatWeapon* pWeapon = pEntity->GetWeapon2();
	IClientEntity* pLocal = (IClientEntity*)Interfaces::EntList->GetClientEntity(Interfaces::Engine->GetLocalPlayer());
	...
	if (pWeapon)
	{
		...
		Color arc = Color(Options::Menu.ColorsTab.Ammoteam.GetValue());
		int ammoyes = pWeapon->GetAmmoInClip() * (size.w) / weapInfo->max_clip;
		Render::outlineyeti(size.x - 1, size.y + size.h + 1, size.w + 2, 4, Color(21, 21, 21, 255));
		Render::rect(size.x, size.y + size.h + 2, size.w, 2, Color(51, 51, 51, 255));
		Render::rect(size.x, size.y + size.h + 2, ammoyes, 2, arc);
...
void CEsp::draw_ghost(IClientEntity* entity, player_info_t pinfo)
{
	Box box;
	...
	if (entity->is_ghost() && hackManager.pLocal()->IsAlive())
	{
		ghost_name(pinfo, box, entity);
	}
}
...
void CEsp::DrawPlayer(IClientEntity* pEntity, player_info_t pinfo)
{
	...
	if (get_box(pEntity, box, Options::Menu.VisualsTab.Active.GetState()) && (pEntity->team() != hackManager.pLocal()->team()))
	{
		...
		if (Options::Menu.VisualsTab.OptionsBox.GetIndex() == 2)
			DrawBoxx2(box, Options::Menu.ColorsTab.BoxCol.GetValue());

		if (Options::Menu.VisualsTab.OptionsBox.GetIndex() == 3)
		...
		if (Options::Menu.VisualsTab.OptionsArmor.GetIndex() == 1)
			DrawVitals_Armor(pEntity, box);

		if (Options::Menu.VisualsTab.OptionsArmor.GetIndex() == 2)
			DrawArmorBar(pEntity, box);
	...
	if (get_box(pEntity, box, Options::Menu.VisualsTab.Active.GetState()) && !Options::Menu.VisualsTab.FiltersEnemiesOnly.GetState() && (pEntity->team() == hackManager.pLocal()->team()))
	{
		Color = GetPlayerColor(pEntity);
		if (Options::Menu.VisualsTab.OptionsBox.GetIndex() == 1)
			DrawBoxx(box, Options::Menu.ColorsTab.BoxColteam.GetValue());

		if (Options::Menu.VisualsTab.OptionsBox.GetIndex() == 2)
			DrawBoxx2(box, Options::Menu.ColorsTab.BoxColteam.GetValue());

		if (Options::Menu.VisualsTab.OptionsBox.GetIndex() == 3)
			Corners(box, Options::Menu.ColorsTab.BoxColteam.GetValue(), pEntity);
		...
		if (Options::Menu.VisualsTab.OptionsSkeleton.GetState())
			DrawSkeleton2(pEntity);
		if (Options::Menu.MiscTab.DebugHitbones.GetState())
			multipoint(pEntity, box);
		if (Options::Menu.VisualsTab.Weapons.GetIndex() == 1) {
			DrawWeapon2(pEntity, box);
		}
		if (Options::Menu.VisualsTab.Weapons.GetIndex() == 2) {
			DrawWeaponIcon(pEntity, box);
		}
...
void CEsp::ghost_name(player_info_t pinfo, Box size, IClientEntity* pEntity)
{
	...
	{
		RECT nameSize = Render::GetTextSize(Render::Fonts::nameaiz, pinfo.name);
		Render::TEXTUNICODE(size.x + (size.w / 2) - (nameSize.right / 2), size.y + size.h + 1, pinfo.name, Render::Fonts::nameaiz, Color(150, 150, 150, 255));
	} 
}
...
void CEsp::DrawArmorBar(IClientEntity* pEntity, Box size)
{
	Box ArmorBar = size;
...
void CEsp::DrawVitals_Armor(IClientEntity* pEntity, Box size)
{
	Box box = size;
	int player_health = pEntity->ArmorValue() > 100 ? 100 : pEntity->ArmorValue();
	if (player_health)
	{
...
void CEsp::DrawSkeleton2(IClientEntity* pEntity)
{
	studiohdr_t* pStudioHdr = Interfaces::ModelInfo->GetStudiomodel(pEntity->GetModel());
	if (!pStudioHdr)
		return;
